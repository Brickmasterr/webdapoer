...
const exportDatabase = async (config, filePath, exportType) => {
    try {
        const dumpOptions = {
            connection: {
                host: config.host,
        ...
        // Dump to file
        dumpOptions.dumpToFile = filePath;

        await mysqldump(dumpOptions);

        console.log(`Database dump (${exportType}) successfully saved to ${filePath}`);
    } catch (error) {
...
exportDatabase(dbConfig, filePath, exportType);